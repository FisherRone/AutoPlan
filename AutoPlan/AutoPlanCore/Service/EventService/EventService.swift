...
enum EventServiceError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .eventPermissionDenied:
            return String(localized: "没有日历访问权限，请在设置中开启。")
        case .reminderPermissionDenied:
            return String(localized: "没有提醒事项访问权限，请在设置中开启。")
        case .eventPermissionRestricted:
            return String(localized: "日历访问受限（可能是家长控制或企业配置）。")
        case .reminderPermissionRestricted:
            return String(localized: "提醒事项访问受限。")
        case .calendarAccessFailed(let error):
            return String(localized: "读取日历数据失败: \(error.localizedDescription)")
        case .saveFailed(let error):
            return String(localized: "保存事项失败: \(error.localizedDescription)")
        case .calendarCreationFailed(let error):
            return String(localized: "无法创建自动计划日历: \(error.localizedDescription)")
        case .sourceNotFound:
            return String(localized: "无法找到可用的日历账户源（如 iCloud），无法创建新日历。")
        }
    }
}
