...
public enum PromptBuilder {
    ...
    /// UserDefaults key for custom weekly report prompt toggle
    private static let useCustomWeeklyReportPromptKey = "useCustomWeeklyReportPrompt"

    /// 读取周报提示词模板（Bundle 内 WeeklyReportPrompt.txt，含 {{...}} 占位符）
    private static let weeklyReportTemplate: String = {
        guard let url = Bundle.module.url(forResource: "WeeklyReportPrompt", withExtension: "txt"),
              let content = try? String(contentsOf: url, encoding: .utf8) else {
            return "{{WEEKLY_REPORT_INSTRUCTION}}\n\n{{CALENDAR_DATA}}"
        }
        return content
    }()

    /// 周报默认指令内容（Bundle 内 WeeklyReportInstruction.txt）
    private static let weeklyReportDefaultInstruction: String = {
        guard let url = Bundle.module.url(forResource: "WeeklyReportInstruction", withExtension: "txt"),
              let content = try? String(contentsOf: url, encoding: .utf8) else {
            return "根据以下数据撰写周报简报。"
        }
        return content
    ...
    /// 自定义周报提示词文件路径
    public static var customWeeklyReportPromptFileURL: URL? {
        guard let appSupport = FileManager.default.urls(
            for: .applicationSupportDirectory, in: .userDomainMask
        ).first else { return nil }
        return appSupport.appendingPathComponent("AutoPlan/custom_weekly_report_prompt.txt")
    }

    /// 确保自定义周报提示词文件存在
    public static func ensureCustomWeeklyReportPromptFileExists() {
        guard let url = customWeeklyReportPromptFileURL else { return }
        let dir = url.deletingLastPathComponent()
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        excludeFromBackup(url: dir)
        if !FileManager.default.fileExists(atPath: url.path) {
    ...
    /// 读取周报模板提示词（供 UI 预览）
    public static var templateWeeklyReportPrompt: String {
        return weeklyReportTemplate
    }

    /// 每次调用时动态读取，根据开关决定使用自定义还是系统默认
    private static func weeklyReportInstruction() -> String {
        let useCustom = UserDefaults.standard.bool(forKey: useCustomWeeklyReportPromptKey)
        if useCustom, let customURL = customWeeklyReportPromptFileURL,
           let content = try? String(contentsOf: customURL, encoding: .utf8),
           !content.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return content
        }
        return weeklyReportDefaultInstruction
    }

    /// 构建周报提示词
    public static func buildWeeklyReportPrompt(
        timeRange: String,
        statistics: WeeklyStatistics,
        calendarMarkdown: String,
        untimedReminders: String
    ) -> String {
        let blocks: [String: String?] = [
            "WEEKLY_REPORT_INSTRUCTION": weeklyReportInstruction(),
            "TIME_RANGE": timeRange,
            "STATISTICS": statistics.summary,
            "CALENDAR_DATA": calendarMarkdown,
            "UNTIMED_REMINDERS": untimedReminders
        ]
        return renderTemplate(weeklyReportTemplate, with: blocks)
    }
}
